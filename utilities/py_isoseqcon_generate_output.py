			...
			if int(exon_number) == 1: # singleton isoform
				...
				if iso_id.startswith("novel_mlt_iso_"): # novel
					if int(read_number) >= mlt_n_lr and fl_c >= mlt_n_fl and (float(flag_ss.split("/")[0])/float(flag_ss.split("/")[1])) >= float(mlt_n_ss):
						...
						print >>output_gpd, "\t".join([gene_id,iso_id,chr,strand,tss,tts,str(fl_c),read_number,exon_number,exon_start,exon_end,derived_genic_loci,derived_genic_loci_over_pct,"NA","NA",flag_ss,subset_iso,str(nuc_A_frq_max),str(nuc_A_pct)])
				else: # known
